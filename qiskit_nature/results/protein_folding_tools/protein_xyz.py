...
class ProteinXYZ:
    ...
    def __init__(
        ...
        self._side_chain_aminoacid_list = np.array(
            [
                aminoacid.residue_sequence[0] if (type(aminoacid) != type(None)) else None
    ...
    def generate_side_positions(self) ->  List[Union[None, np.array]]:
    ...
    def generate_main_positions(self) ->  np.array:
    ...
    def get_xyz_file(self, name: str, output_data: bool) -> np.array:
        ...
        side_aminoacid = np.array(
            self._side_chain_aminoacid_list[self._side_chain_aminoacid_list != None], dtype="<U32"
        )

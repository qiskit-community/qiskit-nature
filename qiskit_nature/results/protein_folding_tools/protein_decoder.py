...
class ProteinDecoder:
    ...
    def __init__(
        self, best_sequence: str, side_chain_hot_vector: List[int], unused_qubits: List[int]
    ) -> None:
    ...
    def get_side_turns(self) -> List[Union[None, int]]:
        ...
        side_turns = self._bitstring2turns(side_turns_bitstring)[::-1]
        result = []
        for element in self._side_chain_hot_vector:
            if element == 1:
                result += [side_turns.pop()]
            else:
                result += [None]
        return result

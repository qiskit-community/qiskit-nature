...
scalar_names_arr = ["VIRIAL", "X-EFIELD", "Y-EFIELD", "Z-EFIELD", "TE SCF ENERGY", "SCRF G-FACTOR",
                    "SCRF A0",
                    "THERMAL ENERGY", "ECC", "ECC(T)", "EVAR1", "ZPE", "COMPOUND ENERGY", "NIMAG",
                    "DEPUHF", "EPUHF", "ECBS2", "ECBSI",
                    "EPMP2", "EPMP3", "GEOM RMSF", "ECIS-MP2", "SCF RMSDP", "S2-A", "ECIS",
                    "DEUMP4D", "EBDREF", "EMP5", "S4SD", "EFC",
                    "SCFTAU", "ESCF", "EUMP2", "EUMP3", "EUMP4", "CBS OIII", "EPRF", "EMP4DQ",
                    "EMP4SDQ", "SCALAR40", "ENUCREP",
                    "PSCFT", "ETOTAL", "S2SCF", "S2-1", "S2-D", "A0", "SCALAR48", "TEMPERATURE",
                    "PRESSURE", "FREQSCALE", "INACTNUCREP",
                    "DE2-SINGLES", "DE2", "RF NUC", "RF ELEC", "CURVATURE", "IRC RC", "EXTFLAG",
                    "ESCF1IT", "JOB STATUS", "NDERIV"]
...
mat_names_arr = ["SHELL TO ATOM MAP", "SHELL TYPES", "NUMBER OF PRIMITIVES PER SHELL",
                 ...
                 "COORDINATES OF EACH SHELL", "BONDS PER ATOM", "BONDED ATOMS", "BOND TYPES",
                 "GAUSSIAN SCALARS",
                 ...
                 "OCTOPOLE INTEGRALS", "HEXADECAPOLE INTEGRALS", "DIP VEL INTEGRALS",
                 "R X DEL INTEGRALS",
                 ...
                 "ALPHA MO DERIVATIVES", "BETA MO DERIVATIVES", "ALPHA SCF DENSITY",
                 "BETA SCF DENSITY",
                 ...
                 "ALPHA QCI/CC DENSITY", "BETA QCI/CC DENSITY",
                 "ALPHA DENSITY CORRECT TO SECOND ORDER",
                 "BETA DENSITY CORRECT TO SECOND ORDER", "ALPHA ONIOM DENSITY",
                 "BETA ONIOM DENSITY",
                 "GIAO D2H/DBDM", "GIAO L/R3", "REGULAR 2E INTEGRALS", "RAFFENETTI 2E INTEGRALS",
                 "AA MO 2E INTEGRALS", "AB MO 2E INTEGRALS", "BA MO 2E INTEGRALS",
                 "BB MO 2E INTEGRALS"]

# scalars in the header records
head_scalars_arr = ["title", "natoms", "nbasis", "nbsuse", "icharg", "multip", "ne", "iopcl",
                    "icgu", "nfc", "nfv", "itran", "idum9", "nshellao", "nprimao", "nshelldb",
                    "nprimdb", "nbondtot"]
...
def rungau(matfi, matfo, program="g16", progargs=[], debug=False, toutput=None, **kwargs):
    ...
    if debug:
        print("rungau program", program, "progargs", pargs, "itemp",
...
class MatEl(object):
    ...
    def print(self, doinp=False, **kwargs):
        ...
        fstr = "NAtoms=" + f6d + sep + "NBasis=" + f6d + sep + "NBsUse=" + f6d + sep + "ICharg=" \
               + f6d + \
        ...
        for lab in mat_names_arr:
            if lab == GSNAME:
                if not qco.doinpprt(GSNAME, self.__GSCAL, doinp=doinp, **kwargs):
                    qco.printlab(GSNAME, 0, 1, 1, len(self.__GSCAL), 0, len(self.__GSCAL), 1, 1, 1,
                                 1, 0, doinp=doinp, **kwargs)
    ...
    def write(self, fname):
        ...
        self.unit = qcmio.open_write(fname, self.labfil, self.gversion, self.title,
                                     self.natoms, self.nbasis, self.nbsuse, self.icharg,
                                     self.multip,
    ...
    def load_head(self, title="No title", natoms=None, nbasis=None, nbsuse=None,
        ...
        if atmwgt is None:
            self.atmwgt = np.array([defatw[min(max(ia, 0), maxan)] for ia in self.ian],
                                   dtype="float64")
        else:

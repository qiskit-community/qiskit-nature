...
def _lenarray(d):
    if len(d) == 5:
        l = d[4] * qcmio.lind4(False, d[0], d[1], d[2], d[3], 0, abs(d[0]), abs(d[1]), abs(d[2]),
                               abs(d[3]))[0] + 1
    elif len(d) == 4:
        l = \
        qcmio.lind4(False, d[0], d[1], d[2], d[3], 0, abs(d[0]), abs(d[1]), abs(d[2]), abs(d[3]))[
            0] + 1
    elif len(d) == 3:
...
def _makeindx(dimens, asym, args):
    if len(dimens) >= 5:
        return qcmio.lind5(True, dimens[0], dimens[1], dimens[2], dimens[3],
                           dimens[4], asym, args[0], args[1], args[2], args[3], args[4])
    elif len(dimens) == 4:
        return qcmio.lind4(True, dimens[0], dimens[1], dimens[2], dimens[3],
                           asym, args[0], args[1], args[2], args[3])
    elif len(dimens) == 3:
        return qcmio.lind3(True, dimens[0], dimens[1], dimens[2], asym, args[0],
                           args[1], args[2])
    elif len(dimens) == 2:
...
def _makeindxc(dimens, asym, args):
    if len(dimens) >= 5:
        ...
        return qcmio.lind3(True, dimens[0], dimens[1], dimens[2], asym,
...
class OpMat(object):
    ...
    def print_mat(self, wid=1, doinp=False, **kwargs):
        ...
        if qcmio.aoints(name):
            ...
            elif (len(self.dimens) >= 4) and (self.dimens[0] == -self.dimens[1]) and (
                    self.dimens[2] == -self.dimens[3]):

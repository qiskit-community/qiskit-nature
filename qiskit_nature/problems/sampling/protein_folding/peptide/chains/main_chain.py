...
class MainChain(BaseChain):
    ...
    def _build_main_chain(self, main_chain_len, main_chain_residue_seq, side_chain_lens,
        ...
        if side_chain_lens is not None and main_chain_len != len(side_chain_lens):
            raise Exception("side_chain_lens size not equal main_chain_len")
        if side_chain_lens is not None and side_chain_lens[0] != 0:
            raise Exception("First main bead is not allowed to have a side chain.")
        for main_bead_id in range(main_chain_len):
            ...
            if side_chain_lens and side_chain_lens[main_bead_id] != 0:

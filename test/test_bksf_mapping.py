...
class TestBKSFMapping(QiskitNatureTestCase):
    ...
    def test_bksf_edge_op_bi(self):
        ...
        ref_qterm_b0 = PauliSumOp.from_list([('IIIZZZ', 1.0)])
        ref_qterm_b1 = PauliSumOp.from_list([('IZZIIZ', 1.0)])
        ref_qterm_b2 = PauliSumOp.from_list([('ZIZIZI', 1.0)])
        ref_qterm_b3 = PauliSumOp.from_list([('ZZIZII', 1.0)])

        self.assertEqual(qterm_b0, ref_qterm_b0, "\n{} vs \n{}".format(
            str(qterm_b0), str(ref_qterm_b0)))
        self.assertEqual(qterm_b1, ref_qterm_b1, "\n{} vs \n{}".format(
            str(qterm_b1), str(ref_qterm_b1)))
        self.assertEqual(qterm_b2, ref_qterm_b2, "\n{} vs \n{}".format(
            str(qterm_b2), str(ref_qterm_b2)))
        self.assertEqual(qterm_b3, ref_qterm_b3, "\n{} vs \n{}".format(
            str(qterm_b3), str(ref_qterm_b3)))
    ...
    def test_bksf_edge_op_aij(self):
        ...
        ref_qterm_a01 = PauliSumOp.from_list([('IIIIIX', 1.0)])
        ref_qterm_a02 = PauliSumOp.from_list([('IIIIXZ', 1.0)])
        ref_qterm_a03 = PauliSumOp.from_list([('IIIXZZ', 1.0)])
        ref_qterm_a12 = PauliSumOp.from_list([('IIXIZZ', 1.0)])
        ref_qterm_a13 = PauliSumOp.from_list([('IXZZIZ', 1.0)])
        ref_qterm_a23 = PauliSumOp.from_list([('XZZZZI', 1.0)])

        self.assertEqual(qterm_a01, ref_qterm_a01, "\n{} vs \n{}".format(
            str(qterm_a01), str(ref_qterm_a01)))
        self.assertEqual(qterm_a02, ref_qterm_a02, "\n{} vs \n{}".format(
            str(qterm_a02), str(ref_qterm_a02)))
        self.assertEqual(qterm_a03, ref_qterm_a03, "\n{} vs \n{}".format(
            str(qterm_a03), str(ref_qterm_a03)))
        self.assertEqual(qterm_a12, ref_qterm_a12, "\n{} vs \n{}".format(
            str(qterm_a12), str(ref_qterm_a12)))
        self.assertEqual(qterm_a13, ref_qterm_a13, "\n{} vs \n{}".format(
            str(qterm_a13), str(ref_qterm_a13)))
        self.assertEqual(qterm_a23, ref_qterm_a23, "\n{} vs \n{}".format(
            str(qterm_a23), str(ref_qterm_a23)))

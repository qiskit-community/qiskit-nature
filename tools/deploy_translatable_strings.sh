...
SOURCE_REPOSITORY="git@github.com:Qiskit/qiskit-nature.git"
SOURCE_DIR=`pwd`
SOURCE_LANG='en'
...
git config user.name "Qiskit (Nature) Autodeploy"
...
echo "copy directory"
cp -r $SOURCE_DIR/$DOC_DIR_PO/ nature/docs
cp $SOURCE_DIR/setup.py nature/.
cp $SOURCE_DIR/requirements-dev.txt nature/.
cp $SOURCE_DIR/requirements.txt nature/.
cp $SOURCE_DIR/qiskit_nature/VERSION.txt nature/qiskit_nature/.Updating the translation deploy script
...
git commit -m "[Qiskit Nature] Automated documentation update to add .po files" -m "skip ci" -m "Commit: $GITHUB_SHA" -m "Github Actions Run: https://github.com/Qiskit/qiskit/runs/$GITHUB_RUN_NUMBER"
echo "git push"
